...
#     sentiment.devX = sentiment.count_vect.transform(sentiment.dev_data)
    ...
    """
    Lemmatization
    # Uncomment to use lemmatization
...
#     sentiment.dev_data[:] = [porter_stemmer.stem(word=i) for i in sentiment.dev_data]
    ...
    sentiment.count_vect = TfidfVectorizer(ngram_range=(1, 2), tokenizer=word_tokenize)
    sentiment.trainX = sentiment.count_vect.fit_transform(sentiment.train_data)
    sentiment.devX = sentiment.count_vect.transform(sentiment.dev_data)
...
if __name__ == "__main__":
    ...
    unlabeled_size = int(unlabeled.X.shape[0]*0.1)
    ...
    for i in range(1):
        unlabeled_temp_Y = classify.evaluate_predict(unlabeled.X, cls)
